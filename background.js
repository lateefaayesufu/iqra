/**
 * Iqra — AI Page Summarizer
 * Background Service Worker (Manifest V3)
 *
 * All AI calls go through the secure Vercel backend.
 * No API key lives in the extension.
 */

'use strict';

// ── Backend URL (loaded from storage dynamically) ───────────
// Set this in the extension's Settings page after deploying to Vercel
...
const IQRA_SECRET = ''; // Optional — set if you added IQRA_SECRET to Vercel
...
function checkRateLimit() {
  const now = Date.now();
  rateLimitTimestamps = rateLimitTimestamps.filter(ts => now - ts < RATE_WINDOW_MS);
  if (rateLimitTimestamps.length >= RATE_LIMIT_MAX) {
    const waitSecs = Math.ceil((RATE_WINDOW_MS - (now - rateLimitTimestamps[0])) / 1000);
    throw new Error(`Rate limit reached. Please wait ${waitSecs}s before summarizing again.`);
  }
...
function buildCacheKey(url, mode) {
  const safe = (url + '|' + mode).replace(/[^a-zA-Z0-9|._~:/?#[\]@!$&'()*+,;=%-]/g, '_').slice(0, 100);
...
function getCachedSummary(url, mode) {
  return new Promise(resolve => {
    ...
    chrome.storage.local.get(key, result => {
      ...
      if (entry && typeof entry === 'object' && Date.now() - entry.ts < CACHE_TTL_MS) {
...
async function callBackend({ content, mode, wordCount, readingTime }) {
  ...
  if (!backendUrl) throw new Error('No backend URL configured. Click ⚙ settings to add your Vercel URL.');
  ...
  if (!res.ok || !data.ok) {
    ...
    if (res.status === 401) throw new Error('Backend auth failed. Check IQRA_SECRET setting.');
    if (res.status === 0 || !res.status) throw new Error('Could not reach the Iqra server. Check your internet connection.');
...
async function summarizePage({ content, url, mode, wordCount, readingTime }) {
  ...
  // Call backend
  const result = await callBackend({ content, mode: safeMode, wordCount, readingTime });
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.action === 'getCacheSize') {
    chrome.storage.local.getBytesInUse(null, bytes => sendResponse({ ok: true, bytes }));
    return true;
  }
  return false;
});
