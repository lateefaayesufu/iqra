 ...
 *  - Sending summarize request to background
 *  - Rendering results
 *  - Mode switching
 *  - Highlight toggle
 *  - Copy / Clear actions
...
async function extractPageContent(tabId) {
  ...
  if (!res || !res.ok) throw new Error(res?.error || 'Could not extract page content.');
  return res.data;
}
...
async function doSummarize() {
  ...
  try {
    ...
    if (!extracted.text || extracted.text.trim().length < 50) {
      throw new Error('Not enough readable content found on this page. Try a different page.');
    }
    ...
    if (!response || !response.ok) {
      throw new Error(response?.error || 'Summarization failed. Please try again.');
    }
...
function copySummary() {
  ...
  const modeLabel = { full: 'Full Summary', '3bullets': '3 Key Bullets', quotes: 'Key Quotes' };
  const lines = [
...
function showToast() {
  ...
  ui.copyToast._timer = setTimeout(() => {
    ...
    setTimeout(() => { ui.copyToast.hidden = true; }, 300);
  }, 1800);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ui.summarizeBtn.addEventListener('keydown', e => {
    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); doSummarize(); }
  });
  ...
  ui.highlightToggle.addEventListener('keydown', e => {
    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleHighlight(); }
  });
