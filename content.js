 ...
 * Responsibilities:
 *  - Extract readable page content using heuristic filtering
 *  - Highlight key phrases on the page
 *  - Remove highlights
 *  - Communicate with popup via chrome.runtime.onMessage
 */
...
(function () {
  ...
  function extractContent() {
    ...
    const noiseSelectors = [
      ...
      '.nav', '.navbar', '.navigation', '.menu', '.site-header',
      '.site-footer', '.page-footer', '.sidebar', '.side-bar',
      '.widget', '.widgets', '.advertisement', '.ads', '.ad',
    ...
    const walker = document.createTreeWalker(
      ...
      NodeFilter.SHOW_TEXT,
      {
        acceptNode(node) {
          ...
          if (['script', 'style', 'textarea', 'input', 'noscript', 'mark'].includes(tag)) {
      ...
      for (const phrase of safePhrases) {
        ...
        try {
          ...
          if (totalHighlights === 1) {
            setTimeout(() => mark.scrollIntoView({ behavior: 'smooth', block: 'center' }), 200);
          }
